...
// Evento para cambiar las opciones
categoriaSelect.addEventListener("change", () => {
  const categoria = categoriaSelect.value;

  // Reiniciamos las opciones del segundo select
  segurosSelect.innerHTML = "<option value=''>-- Selecciona un seguro --</option>";

  if (categoria && opciones[categoria]) {
    opciones[categoria].forEach(seguro => {
      const option = document.createElement("option");
      option.value = seguro.toLowerCase().replace(/\s+/g, "_");
      option.textContent = seguro;
      segurosSelect.appendChild(option);
    });
  }
});
