...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // INICIALIZADOR DEL BOTÓN DE WHATSAPP (¡Aquí está la magia!)
    initWhatsAppButton();
});
...
function initWhatsAppButton() {
    ...
    if (document.getElementById('whatsapp-btn-global')) return;

    // Creamos el enlace <a>
    const waButton = document.createElement('a');
    waButton.id = 'whatsapp-btn-global';
    waButton.href = "https://wa.me/51955067297?text=Hola%20PREVISEGUROS,%20deseo%20más%20información%20sobre%20sus%20seguros.";
    waButton.target = "_blank";
    waButton.rel = "noopener noreferrer";
    
    // Le aplicamos todas las clases de diseño de Tailwind
    waButton.className = "fixed bottom-6 right-6 z-[90] bg-[#25D366] hover:bg-[#1ebd58] text-white p-3 md:p-4 rounded-full shadow-[0_8px_30px_rgba(37,211,102,0.4)] hover:shadow-[0_8px_30px_rgba(37,211,102,0.6)] hover:-translate-y-1 transition-all duration-300 group flex items-center justify-center animate-fade-in-up";
    
    // Agregamos el contenido interno (el tooltip y el icono SVG)
    waButton.innerHTML = `
        <span class="absolute right-16 md:right-20 bg-white text-gray-800 text-xs md:text-sm font-bold py-2 px-4 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none whitespace-nowrap border border-gray-100 hidden sm:block">
            ¡Escríbenos! 👋
        </span>
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" viewBox="0 0 16 16" class="w-8 h-8 md:w-10 md:h-10">
